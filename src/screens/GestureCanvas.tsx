...
export default function GestureCanvas() {
  ...
  const pan = Gesture.Pan()
    ...
    .onTouchesMove((g) => {
      ...
      //   "target": 115,
      // }
      setTGestureMove(`${Math.round(g.changedTouches[0].x)}, ${Math.round(g.changedTouches[0].y)}`)
    })
    .onUpdate((g) => {
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <GestureDetector gesture={pan}>
        <SafeAreaView style={{ flex: 1, backgroundColor: 'black' }}>
          ...
          <Text
            style={{ color: 'white', fontSize: 24 }}>{`Gesture moved to:  ${tGestureMove}`}</Text>
          <Text
            style={{
              color: 'white',
              fontSize: 24,
          ...
          <Text
            style={{ color: 'white', fontSize: 24 }}>{`Gesture ended at:  ${tGestureEnd}`}</Text>
        </SafeAreaView>
      </GestureDetector>
